...
class CartViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        DispatchQueue.main.async { [self] in
            totalMoney = 0.0
            for item in AppManager.shared.arrProducts {
                totalMoney += Double(item.number) * item.newPrices
            }
            myTable.reloadData()
        }
    }
...
extension CartViewController: UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        let footer = FooterCartView().fromNib(nibName: "FooterCartView") as! FooterCartView
        footer.conFig()
        footer.lbPrices.text = "đ\(CustomMoney.numberToMoney(totalMoney))"
        footer.pushCart2VC = { [self] in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        default:
            ...
            cell.numberProducts = { [self] (number : Int) in
                ...
                cell.lbNumber.text = "\(AppManager.shared.arrProducts[indexPath.row].number)"
                totalMoney += item.newPrices * Double(number)
                myTable.reloadData()
            }
            return cell
        }
